...
@inline @pure lowword(d::Float64) = (reinterpret(UInt64, d) >> 32) % UInt32

"""Returns the most significant word of d
Coresponds to OpenLibm GET_HIGH_WORD(ret,d)
"""
@inline @pure highword(d::Float64) = reinterpret(UInt64, d) % UInt32
...
@inline @pure function combinewords(hw::UInt32, lw::UInt32)::Float64
	reinterpret(Float64, (lw % UInt64) << 32 + hw)
end
