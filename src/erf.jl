...
let
  ...
  function erfc1(x::T) where T<:FloatTypes
      s = abs(x) - 1
  ...
  function erfc2(ix::UInt32, x::T) where T<:FloatTypes
  ...
  function erf(x::T) where T<:FloatTypes
      ix = highword(x)
      sign = (ix>>31) % Bool
      ix &= 0x7fffffff
      ...
      if ix < highword(T(0.84375)) # |x| < 0.84375
          ...
          s = @horner z 1 qq1 qq2 qq3 qq4 qq5
          y = r/s
          return x + x*y
      end
  ...
  function erfc(x::T) where T<:FloatTypes
      ix = highword(x)
      sign = (ix>>31) % Bool
      ix &= 0x7fffffff
      ...
      return sign ? 2 - floatmin(T) : floatmin(T)*floatmin(T)
  end
end
